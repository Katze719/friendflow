 ...
 *  `tasksApi`; /me/tasks binds it to `personalTasksApi`. */
export interface TaskClient {
  create(body: CreateTaskPayload): Promise<Task>;
  toggle(taskId: string, done?: boolean): Promise<Task>;
  update(taskId: string, body: UpdateTaskPayload): Promise<Task>;
  remove(taskId: string): Promise<void>;
  clearDone(): Promise<{ removed: number }>;
}

/**
 * Renders the "add task" form. When `group` is null the assignee
 * selector is hidden (personal tasks have no concept of an assignee).
 */
export function AddTaskForm({
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (group) {
        payload.assigned_to = assignee === "" ? null : assignee;
      }
  ...
  // Personal tasks don't carry an assignee, so the layout drops from
  // three columns to two and the selector is hidden outright.
  const gridCols = group ? "sm:grid-cols-3" : "sm:grid-cols-2";
...
export function TaskRow({
  ...
  async function onToggle() {
    ...
    setBusy(true);
    try {
      const updated = await client.toggle(task.id);
      onReplace(updated);
    } catch (e) {
      onReplace(task);
...
function EditTaskForm({
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const updated = await client.update(task.id, payload);
      onSaved(updated);
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : t("common.error"));
    } finally {
